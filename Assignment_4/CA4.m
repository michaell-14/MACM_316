...
%labels to points
for k = 1:length(n)
    text(n(k), iter_c2(k), sprintf('(%d, %d)', n(k), iter_c2(k)), 'VerticalAlignment', 'top', 'HorizontalAlignment', 'left');
end
...
for k = 1:length(n)
    text(n(k), iter_c4(k), sprintf('(%d, %d)', n(k), iter_c4(k)), 'VerticalAlignment', 'top', 'HorizontalAlignment', 'left');
end
